...
writeLog('this is an error', logfile='log/test.log')
"""
...
def writeLog(errormessage, logfile):
    """
    This function writes the customized message to the log file
    """
    self = logs_base(logfile)
    self.createFile(self.logfile)
    full_path = self.getFulldir(self.logfile)
    full_path = full_path.replace("\\",'/')
    log = open(full_path, 'a')
    log.write('[%s]:  %s\n' %(self.getCurrentTime() ,errormessage))
    log.close()
