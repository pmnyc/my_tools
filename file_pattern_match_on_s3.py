...
This Program searches for the pattern of files in the s3 bucket and
    returns the files that match the pattern and also the myid
    associated with the file

Sample Usage:
    > import file_pattern_match_on_s3
    > fileobj = file_pattern_match_on_s3.main(search_file_path_pattern = "mybucket/results/*/ca/*results.json", use_s3key_ind=False)
...
class filePatternMatch(object):
    def __init__(self, search_file_path_pattern, use_s3key_ind=False, s3_key_info="s3_key.json", wildcard="*"):
        self.search_file_path_pattern = search_file_path_pattern
        self.s3_key_info = s3_key_info
        self.wildcard = wildcard
        self.use_s3key_ind = use_s3key_ind
    
    def getStructID(self,x,search_file_path_pattern,wildcard):
        # This function gives myid masked by wildcard by matching x with search_file_path_pattern, in this example
            # the result should be 100000
        ## Sample Input
        # x = "mybucket/result/100000/ca/1000_results.json" #this defines the actual file name
        # search_file_path_pattern = "mybucket/result/*/ca/1000*results.json" #this is the pattern of file for matching
        ##
        ...
        myid = x_split[len(search_short_list)]
        return myid
    ...
    def createListofFilesOnS3(self):
        ...
        use_s3key_ind = self.use_s3key_ind
        ...
        if os.path.exists(os.path.join(os.getcwd(),s3_key_info)) and use_s3key_ind:
        ...
        getfilename = lambda t: "_".join(t.split('/')[-2:])
        self.filename = [str(myid[i]) + "_" + getfilename(filelist[i]) for i in range(len(filelist))]

if __name__ in ['__main__','file_pattern_match_on_s3']:
    def main(search_file_path_pattern, use_s3key_ind=False, s3_key_info="s3_key.json", wildcard="*"):
        # sample input
        # search_file_path_pattern = "mybucket/result/*/ca/1000*results.json"
            # where search_file_path_pattern defines the pattern for searching certain files in s3 bucket
        myobj = filePatternMatch(search_file_path_pattern, use_s3key_ind, s3_key_info, wildcard)
        myobj.createListofFilesOnS3()
        # what we cared about are myobj.filelist & myobj.myid & myobj.filename
        return myobj